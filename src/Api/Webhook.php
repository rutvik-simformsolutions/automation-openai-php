...
class Webhook
{
    public function __construct()
    {
        if (isset($_ENV['PR_REVIEW'])) {
            ...
            $htmlPCR = $this->getPcrHtml($bitbucketService);
            $bitbucketService::generateConfluencePage($htmlPCR, 'OpenAI PCR ' . date('d/m/y H:i'));
        }
    }

    private function getPcrHtml($bitbucketService)
    {
        $pattern = '/AI-\d+/';
        $taskIds = [];
